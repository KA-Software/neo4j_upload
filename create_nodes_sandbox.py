...
def import_orders(session):
    for number in range(1,14):
        command = "LOAD CSV WITH HEADERS FROM \"https://raw.githubusercontent.com/KA-Software/neo4j/main/orders" + str(number) +".csv\" AS row \
                    CREATE (o:Order) \
                    SET o = row, \
...
def import_products(session):
    for number in range(1,6):
        command = "LOAD CSV WITH HEADERS FROM \"https://raw.githubusercontent.com/KA-Software/neo4j/main/products" + str(number) +".csv\" AS row \
                    CREATE (p:Product) \
                    SET p = row, \
...
#126
        ...
        command = "LOAD CSV WITH HEADERS FROM \"https://raw.githubusercontent.com/KA-Software/neo4j/main/order-details" + str(number) +".csv\" AS row \
...
if __name__ == "__main__":
    ...
    "bolt://18.204.9.175:33054", 
    auth=basic_auth("neo4j", "watchstanding-percent-classification"))
    
    stores = "LOAD CSV WITH HEADERS FROM \"https://raw.githubusercontent.com/KA-Software/neo4j/main/store.csv\" AS row CREATE (s:Store) SET s = row, s.storeID=toInteger(row.storeID)"
    suppliers = "LOAD CSV WITH HEADERS FROM \"https://raw.githubusercontent.com/KA-Software/neo4j/main/suppliers.csv\" AS row CREATE (b:Supplier) SET b = row, b.supplierID=toInteger(row.supplierID)"
    customers = "LOAD CSV WITH HEADERS FROM \"https://raw.githubusercontent.com/KA-Software/neo4j/main/customers.csv\" AS row CREATE (c:Customer) SET c = row, c.customerID=toInteger(row.customerID)"
    categories = "LOAD CSV WITH HEADERS FROM \"https://raw.githubusercontent.com/KA-Software/neo4j/main/categories.csv\" AS row CREATE (k:Category) SET k = row, k.categoryID=toInteger(row.categoryID)"
